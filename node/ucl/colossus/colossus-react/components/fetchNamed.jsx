...
function getNamespace(props) {
  return props.match.params.namespace;
}

function isSameNamed(props, nextProps) {
  return getNamespace(nextProps) === getNamespace(props)
    && getName(nextProps) === getName(props);
}

export default function fetchNamed(pluralName) {
  function dispatchAction(dispatch, props) {
    dispatch(fetchNamedAction(getNamespace(props), getName(props), pluralName));
  }
