...
class ClientTableContainer extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      isLoading: false,
  ...
  componentWillReceiveProps(nextProps) {
    // We always reload the list of clients, even if the properties did not change, to avoid
    // showing stale data.
    this.load(nextProps);
  }
  ...
  render() {
    if (this.state.isLoading) {
      ...
      return <ClientTable clients={this.state.data.items} />;
    } else if (this.state.isLoaded) {
      return <NonIdealState
        visual="error"
        title="An error occurred while loading clients."/>;
    }
    return null;
  }
}

ClientTableContainer.propTypes = {
  filter: PropTypes.object,
};

ClientTableContainer.defaultProps = {
  filter: {},
};
